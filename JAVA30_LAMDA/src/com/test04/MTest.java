...
//		createCollection();
	...
	private static void pipeline() {
		...
		List<String> dong = qclass.stream()
								  .filter(name -> name.contains("동"))//true인애들만 보여줄거임
								  .sorted()//정렬
								  .collect(Collectors.toCollection(ArrayList::new));//어레이리스트로 저장할거임
		System.out.println(dong);
		
		//이름이 두글자인 요소만 찾아서
		//새로운 arraylist에 저장해서 리턴하자.
		List<String> two = qclass.stream()
								 .filter(name -> name.length() == 2)
								 .collect(Collectors.toList());
		System.out.println(two);
		
		//이름에 '동'이라는 글자가 있는 사람들을 찾아서
		//성만 그대로 두고, 이름은 나머지로 *로 표시(ex.홍**)하고 => map()
		//list에다가 담은 후에
		//전체 출력하자.
		qclass.stream()
...
//		List<String> list = Arrays.asList(arr);
	...
	private static void createStream() {
		...
		//unary값을 주면 해당값과 같은 타입의 값을 리턴한다.
		//iterator
